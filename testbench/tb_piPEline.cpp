...
void print_config() {
    cout << "using TB" << TB_VPIPELINE << endl;
...
void transaction(VpiPEline* dut, int& send_data_type, Index* index, const vector<vector<vector<int8_t>>>& filter_data,
    ...
    for (int i = 0; i < OFMAP_COL; i++) {
        for (int j = 0; j < OFMAP_CH; j++) {
            if (opsum_data[i][j] != opsum_data_golden[i][j]) {
                ...
                cout << "opsum[" << i << "][" << j << "]: pass" <<setw(6) <<opsum_data[i][j]<< endl;
            }
        }
    }
    if (pass) {
...
int main(int argc, char** argv) {
    ...
    dut->trace(fp, 99);
    fp->open("wave/piPEline_wave.vcd");

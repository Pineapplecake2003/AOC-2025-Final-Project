...
def fuse_conv2d_bias_add_relu_max_pool2d():
    ...
    o = fuse_conv2d_bias_add_relu()

    dequantized_x = dfp.is_op("qnn.dequantize")(o, dfp.wildcard(), dfp.wildcard())

    max_pool2d_op = dfp.is_op("nn.max_pool2d")(dequantized_x)
...
def fuse_dense_add_relu():
    ...
    x = dfp.wildcard()
    dequantized_x = dfp.is_op("qnn.dequantize")(x, dfp.wildcard(), dfp.wildcard())
    dequantized_y = dfp.is_op("qnn.dequantize")(dfp.wildcard(), dfp.wildcard(), dfp.wildcard())
    
    dense_op = dfp.is_op("nn.dense")(dequantized_x, dequantized_y)
    
    dequantized_z = dfp.is_op("qnn.dequantize")(dfp.wildcard(), dfp.wildcard(), dfp.wildcard())
    add_op = dfp.is_op("add")(dense_op, dequantized_z)
...
def fuse_flatten():
    ...
    x = dfp.wildcard()
    dequantized_x = dfp.is_op("qnn.dequantize")(x, dfp.wildcard(), dfp.wildcard())
    flatten_op = dfp.is_op("nn.batch_flatten")(dequantized_x)
    quantize_op = dfp.is_op("qnn.quantize")(flatten_op, dfp.wildcard(), dfp.wildcard())
    cast_op = dfp.is_op("cast")(quantize_op)
    return cast_op

def fuse_global_avg_pool2d():
    # Define the pattern for the operations to be fused
    # Hint: match dequant(i) -> avg_pool2d -> quant -> cast
    # When done, remove the following line

    x = dfp.wildcard()
    dequantized_x = dfp.is_op("qnn.dequantize")(x, dfp.wildcard(), dfp.wildcard())
    global_avg_pool2d_op = dfp.is_op("nn.global_avg_pool2d")(dequantized_x)
    quantize_op = dfp.is_op("qnn.quantize")(global_avg_pool2d_op, dfp.wildcard(), dfp.wildcard())
    cast_op = dfp.is_op("cast")(quantize_op)
    return cast_op
